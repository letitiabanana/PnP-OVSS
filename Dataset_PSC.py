...
class PascalContext(data.Dataset):
    ...
    def __init__(self, root, split, args, nms, processor=None, cam_type=None,vis_processors_clip=None, transform=None, target_transform=None, device="cpu"): #annFile, getClassName, cats,
    ...
    def __getitem__(self, index):
        ...
        '''caption and image preprocessing for bridgetwoer'''
        label_ascap = "A picture of " + " ".join(
            self.nms)  # change the max_txt_length if changing caption, in both this file and blip_image_txt_matching.py file
        # print("64 dataset cocotextloc", label_ascap)
        encoding = self.processor(resized_img, label_ascap, return_tensors="pt")

        # print("66 dataset coco tectloc", encoding.input_ids.shape, encoding.attention_mask.shape, encoding.pixel_values.shape, encoding.pixel_mask.shape)
        for key in encoding.keys():  # one image in processor it would automaticaaly extend the first dim as batch, so need to squeeze that as dataloader would give batch dimension
            encoding[key] = encoding[key].squeeze(0)
        # print("70 ataset coco tectloc", encoding.input_ids.shape, encoding.attention_mask.shape, encoding.pixel_values.shape,
        #       encoding.pixel_mask.shape)
        # transform to tensor at the end to make sure the image input into precessor is in pil image format
        ...
        return  encoding, img_size,  norm_img, img0, path, img_id, label #, gt_class_name  #, target, img
