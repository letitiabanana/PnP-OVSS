    ...
    for batch_id, (norm_img_sizes, norm_img0s, img0s, paths, img_ids) in tqdm(
        ...
        for img_id in img_ids:
            ...
            if args.data_type == "cocoall":
                ...
                target = target_thing
            # print("124 target", len(target), len(target_stuff), len(target_thing))
            # breakpoint()
            ...
            if args.gen_multiplecap_withpnpvqa == 'label':
                ...
                for drop_iter in range(args.drop_iter):
                    ...
                    for img_id in img_ids:
                        ...
                        print("240 attention_path: ", os.path.exists(attention_path))
                        ...
                        if os.path.exists(attention_path) and os.path.exists(highest_att_path):
                            ...
                            run_batch += 0
                        # elif not os.path.exists(clipsim_save_path) and drop_iter > 0:
                        #     print("no clipsim and drop iter >0, run att again")
                        #     run_batch += 1
...
def get_grad_cam_labelascaption(batch_id, drop_iter, args, imgs, imgs_clip_in, norm_img_sizes, norm_imgs, img_ids,
    ...
    #         image_features = clip_features.image_embeds_proj
    #         text_features = clip_features.text_embeds_proj
    #         print("512 text_features, image_features sizes", image_features.size(), text_features.size())
    #         sims = (image_features @ text_features.t()) / 0.01
    #         probs_oneimg_from_clip = torch.nn.Softmax(dim=-1)(sims)#.tolist()
    #         print("515 clip probs", probs_oneimg_from_clip.shape, probs_oneimg_from_clip)
    #         if len(probs_oneimg_from_clip) < 1:
    #             print("517 clip no probs", gt_class_name_list[img_idx], img_ids[img_idx])
    #     ######################## filter label based on clip similarity ######################
    #     captions = []
    #     for i in range(imgs.shape[0]):
    #         pred_gt_class_values, pred_gt_class_idx = probs_oneimg_from_clip[i].topk(30)
    #         print("522 gt_class_idx", pred_gt_class_idx)
    #         pred_gt_class = []
    #         for idx in pred_gt_class_idx:
    #             pred_gt_class.append(nms[idx])
    #         captions.append("An image of " + " and ".join(pred_gt_class))
    #     print("check captions and image id", img_ids, gt_class_name_list, captions)
    #
    # breakpoint()
    ...
    print(
        f"Count 2 {img_ids} Time: run 12 blocks 12 head attention for a batch of images use {toc_grad1 - tic_grad1:0.4f} seconds")
    ...
    for img_idx in range(imgs.shape[0]):
        ...
        if args.ensemble_blocks is not None and "saveall" in args.ensemble_blocks:
            ...
            if args.prune_att_head:
                if args.prune_att_head=="average":
                    max_block_num = args.max_att_block_num
                    att_head = "average"
                    # print("812", gradcam_ensemble[max_block_num-1][0][img_idx].shape)
                    # print("813", len(gradcam_ensemble[max_block_num - 1]), torch.stack(gradcam_ensemble[max_block_num - 1]).shape, torch.stack(gradcam_ensemble[max_block_num - 1]).mean(axis=0)[img_idx].shape)
                    cam_att = torch.stack(gradcam_ensemble[max_block_num - 1]).mean(axis=0)[img_idx]
                    att_loss_record = save_img_union_attention(args, gradcam_id_byclass_dict, gt_class_name,
                                                               att_loss_record,
                                                               img_ids,
                                                               img_idx, drop_iter, norm_img, loss_for_bacth,
