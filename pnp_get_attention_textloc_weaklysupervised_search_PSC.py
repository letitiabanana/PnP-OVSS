        ...
        if args.gen_multiplecap_withpnpvqa == 'label':
            ...
            for drop_iter in range(args.drop_iter):
                ...
                if run_batch > 0:
                    ...
                    print(f"Time: Count Time I Drop patch in {tic1 - toc1:0.4f} seconds")
                    inftime += tic1 - toc1
                    ...
                    get_grad_cam_labelascaption(inftime, batch_id, drop_iter, args, imgs_in, imgs_clip_in, norm_img_sizes,
                                                norm_imgs, img_ids,
                                                gt_class_name_list, captions, model_clip, vis_processors_clip,
                                                model_textloc, text_processors_textloc,
                                                rank, nms, cats)

    print("#########Total inference time:", inftime)
...
def get_grad_cam_labelascaption(inftime, batch_id, drop_iter, args, imgs, imgs_clip_in, norm_img_sizes, norm_imgs, img_ids,
...
def save_img_union_attention(args, gradcam_id_byclass_dict, gt_class_name, att_loss_record, img_ids, img_idx, drop_iter,
    ...
    if drop_iter > 0:
        ...
        if isinstance(args.del_patch_num, int):
            halving_att_sort_patch = np.argsort(sort_union.flatten())[-10:]  # each union att map, get the index of the first 10 highest patch, and save the respective loss for matching image with caption
        elif "sort_thresh" in args.del_patch_num:
