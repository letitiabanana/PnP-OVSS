...
class Attention(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.num_heads = num_heads #16 head for large, 12 for base
    ...
    def forward(self, x, register_hook=False):
        B, N, C = x.shape #C=1024 embedding dimension
        qkv = (
            self.qkv(x)
        ...
        # (2,12,)
        # print("102 vit attn.shape", q.shape, k.shape, v.shape) #([2, 16, 2305, 64]) torch.Size([2, 16, 2305, 64]) torch.Size([2, 16, 2305, 64])
        attn = (q @ k.transpose(-2, -1)) * self.scale
        # print("104 vit attn.shape", attn.shape)
        attn = attn.softmax(dim=-1)
        # print("106 vit attn.shape", attn.shape) #torch.Size([2, 16, 2305, 2305])
        attn = self.attn_drop(attn)

        if register_hook:
            self.save_attention_map(attn)
            self.save_attention_map_withembed((attn @ v))
            attn.register_hook(self.save_attn_gradients)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        # print("114 vit x.shape, attn @ v.shape", x.shape, (attn @ v).shape) #torch.Size([2, 2305, 1024]) torch.Size([2, 16, 2305, 64])
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
