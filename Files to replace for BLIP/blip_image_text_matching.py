...
def compute_gradcam_ensemble(args, model, visual_input, text_input, tokenized_text, drop_iter=0):
    # 改 一次forward , 看register hook
    ...
    for block_num in range(0,12):
        ...
        with torch.inference_mode():
            ...
            gradcams = cams[:, :, :, 1:].reshape(visual_input.size(0), 12, -1, patch_num, patch_num) * grads[:, :, :, 1:].clamp(0).reshape(visual_input.size(0), 12, -1, patch_num, patch_num) * mask[:, :, :cams.shape[2], :, :]
