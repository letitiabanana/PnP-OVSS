...
class BertSelfAttention(nn.Module):
    ...
    def save_attention_map_withembed(self, attention_map_withembed):
        self.attention_map_withembed = attention_map_withembed

    def get_attention_map(self):
        return self.attention_map

    def get_attention_map_withembed(self):
        return self.attention_map_withembed
    ...
    def forward(
        ...
        # print("med 226 cross attention", query_layer.shape, key_layer.shape, value_layer.shape)
        ...
        # print("265 med attention_scores.shape[-1]", attention_scores.shape[-1])
        ...
        # print("268 med attention_probs.shape", attention_probs.shape, torch.matmul(attention_probs, value_layer).shape)

        # if not is_cross_attention:
            
        ### FOR BLIP getting only the cross attention maps
        if is_cross_attention and self.save_attention:
            self.save_attention_map(attention_probs)
            self.save_attention_map_withembed(torch.matmul(attention_probs, value_layer))
            attention_probs.register_hook(self.save_attn_gradients)

        # ### For BLIP2 getting the query to text self attention map and Image to query cross attention
        # self.save_attention_map(attention_probs)
        # self.save_attention_map_withembed(torch.matmul(attention_probs, value_layer))
        # attention_probs.register_hook(self.save_attn_gradients)
